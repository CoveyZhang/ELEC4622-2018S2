...
void my_aligned_image_comp::filter_1(my_aligned_image_comp *in, float sigma)
{
	...
	float *filter_buf_1 = new float[33];
...
void my_aligned_image_comp::filter_2(my_aligned_image_comp *in, float sigma)
{
	...
	// Create the vertical filter PSF as a local array on the stack.
	float *filter_buf_2 = new float[33];
...
main(int argc, char *argv[])
{
	...
	if (argc != 7)
	{
		fprintf(stderr, "Usage: %s <in bmp file> <out bmp prefix> <sigma-minimum> <sigma-maximum> <number> <alpha>\n", argv[0]);
		return -1;
	}
	...
	try {
		...
		float sigma_diff = (log10(sigma_max) - log10(sigma_min)) / (number-1);
		
		//int j = 0;
		float *sigma = new float[10];
		for (float i = 0;i < number;i++) {
			float temp = log10(sigma_min) + sigma_diff*i;
			sigma[int(i)] = pow(10, temp);
			//j++;
		}

		const char* output = ".bmp";
		...
		for (n = 0; n < num_comps; n++)
		{
			intermediea_comps_1[n].filter_2(input_comps + n, sigma[K]);
			intermediea_comps_2[n].filter_1(input_comps + n, sigma[K]);
		}
		for (n = 0; n < num_comps; n++) {
			intermediea_comps_1[n].perform_boundary_extension();
			intermediea_comps_2[n].perform_boundary_extension();
			output_comps[n].out_sum_filter(intermediea_comps_1 + n, intermediea_comps_2 + n,sigma[K],alpha);
		}
		...
		// Write the image back out again
			...
			printf("%s",output);*/
			//char* str = "1.bmp";
			if ((err_code = bmp_out__open(&out, strcat(strcat(name,num), output), out_width, out_height, num_comps)) != 0)
				throw err_code;
			...
			delete[] oline;
			delete[] input_comps;
			delete[] intermediea_comps_1;
			delete[] intermediea_comps_2;
			delete[] output_comps;
			K++;
		}
