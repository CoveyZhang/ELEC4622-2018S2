...
main(int argc, char *argv[])
{
	...
	try {
		...
		for (n = 0; n < num_comps; n++)
			input_comps[n].init(height, width, ceil(3 * sigma));
		...
		for (n = 0; n < num_comps; n++) {
			intermediea_comps_1[n].init(height, width, ceil(3 * sigma));
			intermediea_comps_2[n].init(height, width, ceil(3 * sigma));
			output_comps_0[n].init(out_height, out_width, 1); // Don't need a border for output
			output_comps[n].init(out_height, out_width, 0);
		}
		...
		for (n = 0; n < num_comps; n++) {
			...
			output_comps[n].zerocrossing(output_comps_0 + n);
		}
